...
class PdtamasterElement extends Component {
    ...
    sampleaddmoresave() {
        ...
        if (this.state.buyer.length > 0 && this.state.buyerdiv.length > 0 && this.state.ordercategory.length > 0) {
            ...
            this.setState({ stage: [], activitytype: [], fit: [], activity: '', subactivity: '', days: '', DpndDept: [], DpndOnAct: [], DpndOnSubAct: [], category: [], valueadd: [], mactiveflag: "Y", activeflag: "Y", alowskipflag: "N" })
    ...
    sampleaddmoreedit(item) {
        const { tamasteraddmoredata } = this.state;
        this.setState({
            stage: [{ value: item.stage, label: item.stage }], activitytype: [{ value: item.activityType, label: item.activityType }], fit: [{ value: item.fit, label: item.fitName }],
            ...
            alowskipflag: item.skipped,
            mactiveflag: item.mActive,
            activeflag: item.active,
            DpndOnAct: [{ value: item.dependActvityName, label: item.dependActvityName }],
            DpndOnSubAct: [{ value: item.dependSubActvityName, label: item.dependSubActvityName }],
    ...
    edittnamaster(id) {
        ...
        api.get('TNAMaster/GetTNAMasterListID?ID=' + id)
            .then((response) => {
                ...
                this.setState({
                    hid: id, mactiveflag: data.mActive,
    ...
    render() {
        ...
        for (const item of this.state.DpndOnActlists) {
            ...
            DpndOnActoptions.push({ value: item.activity, label: item.activity });
        }
        ...
        for (const item of this.state.DpndOnSubActlists) {
            ...
            DpndOnSubActoptions.push({ value: item.subActivity, label: item.subActivity });
        }
