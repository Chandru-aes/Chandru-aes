...
class PreprodcutionTable extends Component {
  ...
  getForecastdetail(ForecastId) {
    api
      .get("ForecastQtyDetailEntity/GetForecastGrid?ForecastID=" + ForecastId)
      .then((response) => {
        const values = response.data.data[0];
        this.setState({
          BuyerValue: [{ value: values.buyCode, label: values.buyerName }],
          BuyerdivisionValue: [
            { value: values.buyDivcode, label: values.buyerDivName },
          ],
    ...
    api
      .get("ForecastQtyDetailEntity/GetForecastQtyDetails?FCID=" + ForecastId)
      .then((response) => {
        this.setState({ QtyBreakUpList: response.data });
      });

    api
      .get("ForecastActivityEntity/GetForecastActivityList?FID=" + ForecastId)
      .then((response) => {
        this.setState({
          activityList: response.data,
          activityName:
            response.data.data.length > 0 ? response.data.data[0].activity : "",
          selectedDate:
            response.data.data.length > 0
              ? new Date(response.data.data[0].dueDt)
              : "",
        });
      });
  }
  deleteForecast(ForecastItem, ForecastId) {
    ...
    this.setState({
      deleteForecastdetail: ForecastItem,
      DForecastId: ForecastId,
    });
  }
  deleteForecastConfirm() {
    ...
    api
      .post("ForecastEntity/SaveForecastQtyDetails", deleteForecastHeaderitem)
      .then((response) => {
        this.refs.deleteConfirmationDialog.close();
        NotificationManager.success("Deleted Sucessfully");
        if (response.data.status === true) {
          api.get("ForecastEntity/GetForecastHeaderList").then((response) => {
  ...
  SaveForecast(type) {
    if (
      ...
      if (type === "qty") {
        const payload = Object.assign(saveQtyDetailItems, {
          forecastActivityEntityModel: activityPayload,
        });
        ...
        if (this.state.activityName !== "" && this.state.selectedDate !== "") {
          ...
          const payload = Object.assign(saveActivityItems, {
            forecastActivityEntityModel: activityPayload,
          });
          this.setState({
  ...
  saveForecastDetails = (payload) => {
    api
      .post("ForecastQtyDetailEntity/SaveForecastQtyDetails", payload)
      .then((response) => {
        NotificationManager.success("Added Sucessfully");
        let fcQtyId =
          response.data.data.data.fcQtyDetailInsertEntityModel[0].fcHead_ID;
        api
          .get("ForecastActivityEntity/GetForecastActivityList?FID=" + fcQtyId)
          .then((response) => {
    ...
    api
      ...
      .then((response) => {
        ...
        if (response.data.status === true) {
          ...
          api
            .get(
              "ForecastActivityEntity/GetForecastActivityList?FID=" +
                this.state.currentForecastId
            )
            .then((response) => {
              this.setState({ activityList: response.data });
            });
  ...
  onRowUpdatedActiv(e) {
    ...
    if (fcActivityInsertEntityModel.length > 0) {
      const index = fcActivityInsertEntityModel.findIndex(
      ...
      fcActivityInsertEntityModel.find(function (el, index1) {
        if (el.id == e.data.id) {
          ...
          fcActivityInsertEntityModel[index]["fcHead_ID"] = e.data.fcHead_ID
            ? e.data.fcHead_ID
            : 0;
          ...
          fcActivityInsertEntityModel[index]["cancel"] = "N";
          fcActivityInsertEntityModel[index]["id"] =
            e.data.id > 0 ? e.data.id : 0;
          fcActivityInsertEntityModel[index]["fcHead_ID"] = e.data.fcHead_ID
            ? e.data.fcHead_ID
            : 0;
          fcActivityInsertEntityModel[index]["createdBy"] = "Admin";
      ...
      fcActivityInsertEntityModel[0]["fcHead_ID"] = e.data.fcHead_ID
        ? e.data.fcHead_ID
        : 0;
    ...
    const arrayUniqueByKey = [
      ...new Map(
        fcActivityInsertEntityModel.map((item) => [item[key], item])
      ).values(),
    ];
    ...
    this.state.saveActivityItems = {
      ...
      buyDivcode: this.state.BuyerdivisionValue
        ...
        : e.data.buyDivcode,
      loccode: this.state.location
        ? this.state.location[0].value
        : e.data.loccode,
      seasonCode: this.state.season
        ? this.state.season[0].value
        : e.data.seasonCode,
      seasonYear: this.state.year
        ? this.state.year[0].value
        : e.data.seasonYear,
      fCtype:
        this.state.forecastItem.length > 0
          ? this.state.forecastItem[0].value
          : e.data.fCtype,
      //"fCtype": "SEASONAL",
      createdBy: "1",
      modifyBy: "1",
  ...
  onRowUpdated(e) {
    ...
    if (fcQtyDetailInsertEntityModel.length > 0) {
      fcQtyDetailInsertEntityModel.find(function (el, index1) {
        if (el.id == e.data.id) {
          const index = fcQtyDetailInsertEntityModel.findIndex(
            (item) => item.id === e.data.id
          );
    ...
    const arrayUniqueByKey = [
      ...new Map(
        fcQtyDetailInsertEntityModel.map((item) => [item[key], item])
      ).values(),
    ];
  ...
  getCommonData() {
    ...
    api
      .get("Buyer/GetBuyerDropDown")
      .then((response) => {
        this.setState({ BuyerList: response.data.result.data });
      })
      .catch((error) => {});

    api.get("Location/GetLocationDropDown").then((response) => {
      this.setState({ LocationItem: response.data.result.data });
    });

    api
      .get("Miscellaneous/GetMiscellaneousList?MType=FCTYPE")
      .then((response) => {
    ...
    /*
         ...
         */
    api.get("StyleDivision/GetStyleDivisionList").then((response) => {
      this.setState({ subproductTypes: response.data.result.data });
    });
    api.get("ProductType/GetProductTypeDropDown").then((response) => {
  ...
  getBuyerDivision1(val, field, e) {
    ...
    if (val && val.BuyerValue.length > 0) {
      ...
      api
        .get(
          "BuyerDivision/GetBuyerDivisionDropDown?BuyerID=" +
            val.BuyerValue[0].value
        )
        .then((response) => {
          this.setState({
            BuyerdivisionValue: [],
  ...
  getSubproductType(options) {
    return {
      ...
      filter: options.data
        ? ["productType", "=", options.data.productType]
        : null,
    };
  }
  ...
  render() {
    ...
    for (const item of this.state.BuyerList) {
      BuyerOptions.push({
        ...
        label: item.buyerCode + " - " + item.buyerName,
      });
    }
    ...
    for (const item of this.state.BuyerDivisionList) {
      BuyerDivisionOptions.push({
        ...
        label: item.divisionCode + " - " + item.divisionName,
      });
    }
    ...
    return (
      <div className="formelements-wrapper main-layout-class">
        ...
        <Accordion>
          <AccordionSummary
            expandIcon={<i className="zmdi zmdi-chevron-down" />}
          >
            ...
            <div className="col-sm-12 col-md-12 col-xl-12 p-0">
              <RctCollapsibleCard heading="">
                    ...
                    <div className="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                      <div className="form-group">
                        <div className="form-group select_label_name mt-15">
                          ...
                          />
                          <span className="error">
                            {this.state.errors["forecastItem"]}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  >
                    <TabContainer>
                      <div className="">
                        <div id="data-grid-demo">
                          <DataGrid
                            dataSource={this.state.QtyBreakUpList.data}
                            keyExpr=""
                            showBorders={true}
                            onRowUpdated={this.onRowUpdated}
                            onRowRemoving={this.onRowRemoving}
                            onRowInserting={this.onRowUpdated}
                          >
                            ...
                            <Editing
                              ...
                              selectTextOnEditStart={
                                this.state.selectTextOnEditStart
                              }
                            ...
                            >
                              <Lookup
                                dataSource={this.state.productTypes}
                                valueExpr="productType"
                                displayExpr="productType"
                              />
                              <RequiredRule />
                            </Column>
                            <Column
                              dataField="subProductType"
                              caption="Sub-Product type"
                            >
                              {/* */}
                              <Lookup
                                dataSource={this.getSubproductType}
                                valueExpr="subProductType"
                                displayExpr="subProductType"
                              />
                              <RequiredRule />
                            </Column>
                            <Column
                            ...
                            <Column dataField="PCD" dataType="date">
                              <RangeRule
                                message="Past dates cannot be allowed"
                                min={new Date()}
                              />
                              <CustomRule
                                type="custom"
                                validationCallback={(e) =>
                                  this.dateValidation(e, "pcd")
                                }
                              />
                              <RequiredRule />
                            </Column>
                            ...
                            >
                              <RangeRule
                                message="Should be greater than or equal to PCD date."
                                min={this.state.pcdDate}
                              />
                              <CustomRule
                                type="custom"
                                validationCallback={(e) =>
                                  this.dateValidation(e, "tentative")
                                }
                              />
                              <RequiredRule />
                            </Column>
                            ...
                            >
                              <RangeRule
                                message="confirmation due dates should be before or equal to tentative delivery date"
                          ...
                          <div className="w-50 float-right mt-20">
                            ...
                            <div className="w-30 float-left">
                              <button
                                className="float-left MuiButtonBase-root MuiButton-root MuiButton-contained  mr-10  btn-icon b-ic"
                                tabindex="0"
                                type="button"
                              >
                          ...
                          <div className="w-100">
                            <div className="float-right">
                              <button
                                ...
                                tabindex="0"
                                type="button"
                                onClick={(e) => this.contactSubmit(e, "qty")}
                              >
                                <span className="MuiButton-label">
                                  Save <i className="zmdi zmdi-save" />
                                </span>
                                <span className="MuiTouchRipple-root" />
                              </button>
                    ...
                    <TabContainer>
                        ...
                        <div className="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                          <div className="form-group">
                            <TextField
                              ...
                              onChange={this.setstatevaluefunction(
                                "activityName"
                              )}
                            />
                            <span className="error">
                      ...
                      <div className="table-responsive">
                        <DataGrid
                          dataSource={this.state.activityList.data}
                          keyExpr="id"
                          showBorders={true}
                          onRowUpdated={this.onRowUpdatedActiv}
                          onRowRemoving={this.onRowRemovingActiv}
                          onRowInserting={this.onRowUpdatedActiv}
                        >
                          <Paging enabled={false} />
                          <Editing
                            mode="cell"
                          ...
                          <Column dataField="dueDt" dataType="date">
                            <RequiredRule />
                          </Column>
                        </DataGrid>
                      </div>
                        ...
                        <div className="w-100">
                          <div className="float-right">
                            <button
                              ...
                              tabindex="0"
                              type="button"
                              onClick={(e) => this.contactSubmit(e, "activity")}
                            >
                              <span className="MuiButton-label">
                                Save <i className="zmdi zmdi-save" />
                              </span>
                ...
                >
                  ...
                  <ModalBody>
                    <AddNewUserForm
                      addNewUserDetails={this.state.age}
                      onChangeAddNewUserDetails={this.handleChange.bind(this)}
                    />
                  </ModalBody>
                  <ModalFooter>
                    <Button
                ...
                <Modal
                  isOpen={this.state.addQuantityModal}
                  toggle={() => this.onAddUpdateUserModalClose()}
                >
                  <ModalHeader toggle={() => this.onAddUpdateUserModalClose()}>
                    Quantity Breakup
                ...
                <Modal
                  isOpen={this.state.guideformmodal}
                  toggle={() => this.onAddUpdateUserModalClose()}
                  className="modal-lg"
                >
                  <ModalHeader toggle={() => this.onAddUpdateUserModalClose()}>
              ...
              <Accordion defaultExpanded>
                ...
                <AccordionDetails>
                  <div className="float-right tbl-filter-btn">
                    ...
                    <button
                      className="MuiButtonBase-root MuiIconButton-root jss26"
                      tabindex="0"
                      type="button"
                          ...
                          <tr>
                            <td className="text-center">
                              ...
                              <button
                                ...
                                type="button"
                                aria-label="Delete"
                                onClick={(e) => this.getForecastdetail(n.id)}
                              >
                                <span className="MuiIconButton-label">
                              ...
                              <button
                                ...
                                type="button"
                                aria-label="Delete"
                                onClick={(e) => this.deleteForecast(n, n.id)}
                              >
                                <span className="MuiIconButton-label">
                              ...
                              <button
                                ...
                                type="button"
                                aria-label="Clone"
                                onClick={(e) => this.getForecastdetail(n, n.id)}
                              >
                                <span className="MuiIconButton-label">
